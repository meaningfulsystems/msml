...
| [projects/apollo/apollo-model.msml](../../projects/apollo/apollo-model.msml) | Full system + subsystem. Apollo 11 / Block II, AS-506. Sourced vehicle/AGC/electrical/AGS/docking/RCS (SM/LM 100 lbf, CM 93 lbf). UNKNOWN: official CSM lunar Δv, SPS loaded lb, AGS program name, loaded SM/CM RCS propellant. |
...
**States.** State definitions plus `transition` relationships. E-bike `RideControl` states: `off`, `standby`, `assist`, `walk` (≤ 6 km/h, EPAC, not throttle), `charging`, `fault`. `resetFault` is **Fault→Off**, not Fault→Standby. Prefer **Standby→Charging** (`plugInStandby`) in addition to Off→Charging.
