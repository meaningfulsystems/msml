...
| [projects/apollo/apollo-model.msml](../../projects/apollo/apollo-model.msml) | Full system + subsystem. Apollo 11 / Block II, AS-506. Sourced vehicle/AGC/electrical/AGS/docking/RCS (SM/LM 100 lbf, CM 93 lbf). Mission STM: TLI → dockEject → translunar → LOI (not translunar then dockEject). TDE ~03:20–04:09 planned; LOI-1 75:54:28 A11-FP planned (do not cite Press Kit as the LOI-1 source); flown ~075:49:50 (PAD/MR). TLI three labels (do not collapse): PK planned 02:44:15; A11-FP planned 2:44:26; flown 02:44:16 (MSC-00171). SPS on SM. landingRadar on descent (three-beam, P63–P64); rendezvousRadar on ascent. D-7720 April 1967 food 2800 CM / 3200 LM (not A11 flown). Crew Safety not LES-only. UNKNOWN: official CSM lunar Δv, SPS loaded lb, AGS program name, loaded SM/CM RCS propellant. |
...
**Requirements.** Keep them **siblings** under the system unless the user asks for a real refine. Ids in the e-bike are frozen: `rideSafetyRequirement`, `rangeRequirement`, `assistLimitRequirement`, `chargeSafetyRequirement`, `brakeOverrideRequirement`, `batteryCutoffRequirement`, `displayRequirement`, `structuralRequirement`, plus siblings `motorAssistCutoffRequirement` (EN 15194:2017 4.2.13 Power management: motor-assist cut-off after pedaling stops is 2 m, or 5 m if brake-lever switches relax it — not vehicle brake distance; allocate to controller + cadence + wheel-speed, not BrakeSystem), `lightingRequirement` (StVZO / ISO 6742), `walkAssistRequirement` (≤ 6 km/h), and `continuousPowerRequirement` (250 W). 50 ms is an electronic inhibit budget on brake override only. Display and Structural are not children of Range or Assist Limit. Do **not** hang walk or 250 W under Assist Limit (a leftover SysML2d refine). Brake Override may refine Ride Safety. Range 500 Wh / 60 km is Tour-mode via `usableWh` / `energyPerKm` (~8.3 Wh/km), not Eco / PAS-1. `allocateChargeToBms` targets the nested usage `ElectricBike.BatteryPack.bms` (`ElectricBike::BatteryPack::bms`). Allocate Assist Limit to the `wheelSpeedSensor` usage as well as the controller. Usages: `cadenceSensor`, `wheelSpeedSensor`. Show nested `bms : BMS` on the IBD charge path when readable. Hub motor: `continuousAssist` 250 W is EU continuous (not peak); `peakTorque` 40 N·m is hub peak (not continuous).
