...
**States.** State definitions plus `transition` relationships. E-bike `RideControl` states: `off`, `standby`, `assist`, `walk` (≤ 6 km/h, EPAC, not throttle), `charging`, `fault`. `resetFault` is **Fault→Off**, not Fault→Standby. Charging from `Off` only — an EPAC choice, labeled on the Charging state; do **not** add `Standby` → `Charging`.
