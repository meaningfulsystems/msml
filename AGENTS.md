...
- Mission STM: countdown → boost → earthOrbit (100 nmi **planned**) → TLI → dockEject → translunar → LOI → undock → DOI → descent → surface/EVA → ascent → rendezvous → TEI → entry → recovery. `dockEject` is its own state (CMP-owned, SM RCS, probe-drogue). Sequence lock with SysML2d: **TLI → dockEject → translunar → LOI** (not translunar then dockEject). Do **not** jump TLI → translunar or dockEject → LOI. GET: planned vs flown — earth orbit 100 nmi is planned; A11 **flown** TLI 02:44:15, TDE ~03:20–04:09, LOI-1 75:54:28 GET. IU owns boost+TLI. RSO owns destruct until orbital safing. Abort machine in parallel: pad/LES, Modes I–IV, contingency TLI, SPS abort, lunar abort (P70 DPS / P71 APS). SPS is on the SM. landingRadar is on the descent stage, not PNGS / ascent.
