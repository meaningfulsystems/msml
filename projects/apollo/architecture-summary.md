...
NASA NPR 7123.1 (NASA Procedural Requirements 7123.1, NASA Systems Engineering Processes and Requirements) is the civil systems-engineering process this walkthrough teaches in plain language. MagicGrid layers map to that process as follows — names stay MagicGrid, not NPR product titles:

| MagicGrid layer | NPR 7123.1 idea (plain language) |
| --- | --- |
| Problem / context | Stakeholder expectations — who needs what from the mission |
| Requirements and use cases | Technical requirements — the shalls that implement those expectations |
| Structure | Logical decomposition, then the design solution (the parts that realize the functions) |
| Behavior | How the design solution operates in time |
| Parametrics | Analyses that support the design (sourced numbers only) |
| Allocations | Product verification / validation trace — which part satisfies which shall |
| Open risks / unmarked | What the sources do not close — do not invent |
...
Sources used on the model include Apollo 11 Press Kit 69-83K, Saturn V Flight Manual extracts, Apollo Experience Reports, AGCIS / MIT Instrumentation Laboratory, and Technical Notes (TN) D-6718 / D-6724 / D-7082 / D-7143 / D-7375 / D-8093 / TN-7990 / D-7720. Values not in those extracts stay unmarked.

## 2. Requirements and use cases

Technical requirements on the model (NPR: the shalls that implement stakeholder expectations):
...
| `Apollo.usbRfRequirement` (REQ-007) | USB RF (sourced) | CSM **2106.40625** ↑ / **2287.5** Phase Modulation (PM) ↓ / **2272.5** Frequency Modulation (FM); LM **2101.802** ↑ / **2282.5**. Pulse-Code Modulation (PCM) 51.2 or 1.6 kbps. Uplink digital ~2 kbps. Pseudo-Random Noise (PRN) range 992 kbps, ±15 m, ~540,000 mi unambiguous. |
| `Apollo.p27Requirement` (REQ-008) | P27 uplink verbs | P27 uplink verbs **only V70–V73** into Command Module Computer (CMC) / LM Guidance Computer (LGC). Separate from the Command, Communications, and Telemetry System (CCATS) command-load path. |
...
Logical decomposition, then the design solution (NPR): the stack parts that realize those functions.

Pad stack: S-IC-6, S-II-6, S-IVB-6N, IU-6, SLA-14 (**8 panels: 4 jettison / 4 stay**; LM-5), SM (owns **SPS**), CM, LES.

**SPS is on the SM, not the CM.** Service Propulsion System (SPS) composition is `Apollo.SM` → `Apollo.SPS`. The CSM IBD nests `sps` inside `sm`. Do not hang SPS on the command module.

**Landing radar is on the LM descent stage** (three-beam, P63–P64), not ascent Primary Guidance and Navigation System (PNGS). Composition is `Apollo.Descent` → `Apollo.LandingRadar`. **Rendezvous radar is on the ascent stage.** Composition is `Apollo.Ascent` → `Apollo.RendezvousRadar`. PNGS stays on the LM; AGC_LM stays under PNGS. Do not park landing radar under PNGS on the ascent tree.
...
- AGC_LM (Luminary 1A) owns landing / ascent / LM abort (P63–P68 land; P70 Descent Propulsion System (DPS) / P71 Ascent Propulsion System (APS)).
- AGS (AEA 4096×18, 5 µs) is abort-to-orbit / rendezvous only. DEDA is not a DSKY.

Electrical: SM three fuel cells on A11 2 H2 + 2 O2 cryo; CM three AgZn + Lower Equipment Bay (LEB) charger + separate pyro; LM four descent + two ascent AgZn with an Electronic Control Assembly (ECA) on each battery. No stage-to-stage electrical power.
...
Cite **both** SPS figures: 20,500 lbf (PK) vs 21,500 lbf vac (TN D-7375). Cite **both** DPS figures: 9,870 / 1,050–6,300 lbf (PK) vs 10,500 lbf and 10:1 (TN D-7143). USB and VHF stay as already on the model (section 2 and 3).

The ECLSS parametric view binds the sourced CM / LM-5 / A7L numbers and the D-7720 food plan. A11 actual kcal and entry blackout stay UNKNOWN.
...
Product verification / validation trace (NPR): which part satisfies which shall.
...
Descent O2 tank pressure is cited both ways (2800 vs 3000 psi, TN D-6724). Tank loads stay **UNRECONCILED**, same flag as Δv.

## Generated views

These figures illustrate the architecture above; they do not replace it. Every rendered figure in this folder is listed.
...
**`apollo-bdd.png` — block definition diagram (BDD).** Saturn V + CSM + LM + LES + SLA at system grain.
...
**`apollo-act.png` — mission activity.** Same phases as a start-to-recovery control flow.
...
**`apollo-sat-ibd.png` — Saturn V IBD.** Stack joints + IU LVDC. No stage-to-stage electrical power.
...
**`apollo-lm-bdd.png` — LM BDD.** Descent + landing radar; ascent + rendezvous radar; DPS/APS; AGC_LM + one DSKY; AGS.

**`apollo-lm.png` — LM IBD.** PNGS, AGC_LM, AGS, descent + landing radar (three-beam), ascent + rendezvous radar, DPS, APS.
...
**`apollo-gnd-bdd.png` — ground BDD.** MCC-H / GSFC / MSFN / LC-39 / Recovery, consoles, AIS, ARIA.

**`apollo-gnd.png` — ground IBD.** Facility wiring. RSO/AFETR outside MCC. UNKNOWN markers stay visible.

**`apollo-crew.png` — crew IBD.** CDR / CMP / LMP each in an A7L suit. PLSS + Oxygen Purge System (OPS) on CDR/LMP EVA only.

**`apollo-usb.png` — USB comms.** Sourced RF. Crew-selected antennas. P27 ≠ CCATS.
...
**`apollo-ags-bdd.png` — AGS BDD.** AEA + ASA (strapdown) + DEDA (not a DSKY). Sourced AEA memory.
