...
Mission STM: countdown → boost → earthOrbit (100 nmi planned) → TLI → dockEject → translunar → LOI → undock → DOI → descent → surface/EVA → ascent → rendezvous → TEI → entry → recovery. `dockEject` is its own state (CMP-owned, SM RCS, probe-drogue). Do **not** put translunar before dockEject, and do not jump TLI → translunar or dockEject → LOI. TDE ~03:20–04:09 GET is planned. LOI-1 75:54:28 GET is A11-FP planned (do not cite Press Kit as the LOI-1 source); flown LOI-1 is ~075:49:50 GET. TLI three labels: PK planned 02:44:15; A11-FP 2:44:26; flown 02:44:16. IU owns boost+TLI; RSO owns destruct until orbital safing.
