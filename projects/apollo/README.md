...
Namespace `Apollo`. File stem `apollo`. Layout is `projects/apollo/`. MagicGrid architecture walkthrough: [architecture-summary.md](architecture-summary.md).
...
| CSM IBD | `apollo-csm` | CM GNC · SM owns SPS · RCS · ECLSS |
| LM BDD | `apollo-lm-bdd` | descent + landing radar · ascent + rendezvous radar · DPS/APS · AGC_LM + 1 DSKY · AGS |
| LM IBD | `apollo-lm` | PNGS · AGC_LM · AGS · descent + landing radar · ascent + rendezvous radar · DPS · APS |
...
| ECLSS parametric | `apollo-eclss-par` | Sourced CM / LM-5 / A7L numbers; D-7720 food plan; A11 intake and blackout UNKNOWN |
...
Mission STM: countdown → boost → earthOrbit (100 nmi planned) → TLI → dockEject → translunar → LOI → undock → DOI → descent → surface/EVA → ascent → rendezvous → TEI → entry → recovery. `dockEject` is its own state (CMP-owned, SM RCS, probe-drogue). Do **not** put translunar before dockEject, and do not jump TLI → translunar or dockEject → LOI. TDE ~03:20–04:09 GET is planned, not flown. LOI-1 75:54:28 GET is A11-FP planned; flown LOI-1 is ~075:49:50 GET (PAD/MR). TLI three labels (do not collapse): PK planned 02:44:15; A11-FP planned 2:44:26; flown 02:44:16 (MSC-00171). Do not call TLI 02:44:15, TDE, or LOI-1 75:54:28 flown. IU owns boost+TLI; RSO owns destruct until orbital safing.

Abort machine (parallel): pad/LES, Modes I–IV, contingency TLI, SPS abort, lunar abort (P70 DPS / P71 APS).

Educational estimate scripts live in [simulations/](simulations/). They print `ESTIMATE` ranges from sourced masses. Simulation output is not a NASA fact. Do not copy those numbers into this README, [architecture-summary.md](architecture-summary.md), or any shall. Change-in-velocity, SPS loaded mass, and SM/CM RCS propellant stay unmarked on the architecture notes.
