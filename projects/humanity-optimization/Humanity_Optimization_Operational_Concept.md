...
MSML combines those ideas into a JSON-based graphical modeling format where each diagram is a file, every element has explicit layout, and the result can be rendered into PNG images. In other words, it is not just a drawing. It is a model that can be scripted, reviewed, changed, regenerated, and eventually validated.
